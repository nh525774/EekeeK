...
const CommentItem = ({ item, canDelete = false, onDelete, displayText, scanning }) => {
  ...
  const renderWithMaskAndMentions = () => {
    // ⬇️ 2안: displayText가 오면 그걸 그대로 사용(마스킹 반영본)
    if (typeof displayText === "string") {
      return renderMentions(displayText);
    }

    // (호환) 예전 pieces 기반 렌더 유지
    const pieces = item?.pieces;
    if (!Array.isArray(pieces) || pieces.length === 0) {
      return renderMentions(item?.renderedText || item?.text || "");
    }
    return pieces.map((p, idx) =>
      p.masked ? (
        <span key={`pm-${idx}`} className="pii-masked" title="민감정보 보호됨">
          {p.text}<span className="pii-lock" aria-hidden />
        </span>
      ) : (
        <React.Fragment key={`pn-${idx}`}>{renderMentions(p.text || "")}</React.Fragment>
      )
    );
  };
